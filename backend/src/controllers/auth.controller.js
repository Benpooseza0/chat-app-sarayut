...
export const signup = async (req, res) => {
  ...
  try {
    ...
    await newUser.save(); // บันทึกก่อน
    generateToken(newUser._id, res); // แล้วค่อยสร้าง token
...
export const login = async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      return res.status(404).json({ message: "กรุณากรอกอีเมล์ให้ถูกต้อง" });
    }

    const isPasswordCorrect = await bcrypt.compare(password, user.password);
    if (!isPasswordCorrect) {
      return res.status(400).json({ message: "กรุณากรอกรหัสผ่านให้ถูกต้อง" });
    }
    ...
    res.status(500).json({ message: "เซิร์ฟเวอร์เกิดข้อผิดพลาด" });
  }
};
